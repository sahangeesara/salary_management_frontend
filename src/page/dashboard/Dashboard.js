...
export default function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([
      safe(() => EmployeeService.getAllEmployees()),
      safe(() => PayrollService.getAllPayroll()),
      safe(() => BonusService.getAllBonus()),
      safe(() => AllowanceService.getAllAllowance()),
      safe(() => DeductionService.getAllDeduction()),
      safe(() => OvertimeService.getAllOvertime()),
      safe(() => LeaveService.getAllEmployeeLeave()),
  ...
  /* ── Spark data (mock monthly distribution) ── */
  ...
  return (
    <div className="dash-page">
      ...
      {/* ══ Row 2: Payroll breakdown + Dept chart ══ */}
      ...
      {/* ══ Row 3: Recent employees + Leave + Quick summary ══ */}
      <div className="dash-row3">
        ...
        <div className="dash-card leave-card">
          ...
          <div className="mini-card orange">
            <div className="mini-card-icon"><i className="bi bi-clock-history" /></div>
            <div>
              <div className="mini-card-val">{overtimes.length}</div>
              <div className="mini-card-label">Overtime Entries</div>
            </div>
            <div className="mini-card-amount">{fmtK(totalOvertime)}</div>
          </div>

          {/* Allowance mini card */}
          <div className="mini-card purple">
            <div className="mini-card-icon"><i className="bi bi-plus-circle-fill" /></div>
            <div>
              <div className="mini-card-val">{allowances.length}</div>
              <div className="mini-card-label">Allowance Records</div>
            </div>
            <div className="mini-card-amount">{fmtK(totalAllowance)}</div>
          </div>
        </div>
